...
@Component({
    ...
    template: `
        <aside [class]="open() ? '' : '-translate-x-full'" class="bg-linear-to-b from-gray-900 via-gray-800 to-gray-900 w-64 top-0 min-h-screen fixed text-white transition-all duration-300 ease-in-out overflow-hidden flex flex-col h-full shadow-2xl z-50">
            <!-- Logo Section -->
            <div class="px-4 py-6 border-b border-gray-700/50">
                <div class="flex items-center justify-center gap-3">
                    <img src="assets/images/LOGO IRONFIT SIN LET.png" alt="Iron Fit" class="w-10 h-10">
                    <span class="text-xl font-bold bg-linear-to-r from-orange-400 to-red-500 bg-clip-text text-transparent">
                        Iron Fit
                    </span>
                </div>
            </div>
                        ...
                        <li>
                            @if (section.children) {
                                ...
                                <button 
                                    ...
                                    class="w-full flex items-center justify-between gap-3 px-4 py-3 rounded-lg transition-all duration-200 hover:bg-gray-700/50 group">
                                    <div class="flex items-center gap-3">
                                        <span [innerHTML]="section.icon" class="w-5 h-5 text-gray-400 group-hover:text-white transition-colors"></span>
                                        <span class="font-medium text-sm">{{ section.label }}</span>
                                    </div>
                                    <svg 
                                        [class]="expandedSections().has(section.label) ? 'rotate-180' : ''"
                                        ...
                                        @for(child of section.children; track child.path) {
                                            <li>
                                                <a 
                                                    ...
                                                    routerLinkActive="bg-orange-600/20 text-orange-400 border-l-2 border-orange-500"
                                                    [routerLinkActiveOptions]="{exact: false}"
                                                    class="flex items-center gap-3 px-4 py-2 rounded-lg transition-all duration-200 hover:bg-gray-700/50 group text-sm">
                                                    <span [innerHTML]="child.icon" class="w-4 h-4 text-gray-400 group-hover:text-white transition-colors"></span>
                                ...
                                <a 
                                    ...
                                    routerLinkActive="bg-linear-to-r from-orange-500 to-red-600 shadow-lg"
                                    [routerLinkActiveOptions]="{exact: false}"
                                    class="flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 hover:bg-gray-700/50 group">
                                    <span [innerHTML]="section.icon" class="w-5 h-5 text-gray-400 group-hover:text-white transition-colors"></span>
                                    <span class="font-medium text-sm">{{ section.label }}</span>
                                </a>
                            }
            ...
            <div class="p-4 border-t border-gray-700/50">
                <div class="text-xs text-gray-400 text-center">
                    ...
                    <p class="text-gray-500 mt-1">{{ roleLabel() }}</p>
                </div>
            </div>
        </aside>
    `,
...
export class DashboardSidebarComponent {
    ...
    expandedSections = signal<Set<string>>(new Set(['Gestión de Usuarios', 'Catálogos', 'Planes y Rutinas', 'Reportes']));
    ...
    adminSections: MenuSection[] = [
        ...
        },
        {
            ...
            children: [
                {
                    label: 'Ejercicios',
                    path: '/admin/catalog/exercises',
                    icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v3a3 3 0 003 3z"/></svg>'
                },
                {
                    label: 'Categorías',
                ...
                },
                {
                    ...
                    path: '/admin/catalog/muscle-groups',
                ...
                {
                    label: 'Objetivos de Planes',
                    path: '/admin/catalog/objectives',
                    icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4M7.835 4.697a3.42 3.42 0 001.946-.806 3.42 3.42 0 014.438 0 3.42 3.42 0 001.946.806 3.42 3.42 0 013.138 3.138 3.42 3.42 0 00.806 1.946 3.42 3.42 0 010 4.438 3.42 3.42 0 00-.806 1.946 3.42 3.42 0 01-3.138 3.138 3.42 3.42 0 00-1.946.806 3.42 3.42 0 01-4.438 0 3.42 3.42 0 00-1.946-.806 3.42 3.42 0 01-3.138-3.138 3.42 3.42 0 00-.806-1.946 3.42 3.42 0 010-4.438 3.42 3.42 0 00.806-1.946 3.42 3.42 0 013.138-3.138z"/></svg>'
                },
                {
                    label: 'Estados de Planes',
                    path: '/admin/catalog/states',
                    icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z"/></svg>'
                }
            ]
        },
        {
            label: 'Planes y Rutinas',
            ...
            children: [
                {
                    ...
                    path: '/admin/routines/training-plans',
    ...
    adminLinks: SidebarLink[] = [
        ...
        {
            label: 'Progreso por Sesiones',
            path: '/admin/reports/progress',
            icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"/></svg>'
        }
    ];

    trainerLinks: SidebarLink[] = [
        {
            label: 'Dashboard',
    ...
    trainerSections: MenuSection[] = this.trainerLinks.map(link => ({ ...link, path: link.path }));
    clientSections: MenuSection[] = this.clientLinks.map(link => ({ ...link, path: link.path }));

    menuSections = computed(() => {
        const role = this.sessionHandler.role();

        if (role === 'AUDITOR') return this.adminSections;
        if (role === 'TRAINER') return this.trainerSections;
