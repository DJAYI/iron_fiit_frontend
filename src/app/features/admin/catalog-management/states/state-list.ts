    ...
    template: `
        <div class="max-w-7xl mx-auto">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-3xl font-bold text-gray-800">Estados de Planes de Entrenamiento</h1>
                <button 
                    (click)="onCreate()"
            ...
            @if (loading()) {
                <div class="text-center py-12">
                    ...
                    <p class="mt-4 text-gray-600">Cargando estados...</p>
                </div>
            } @else {
                @if (states().length === 0) {
                                ...
                                @for (state of states(); track state.id) {
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900">{{ state.id }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
...
export class StateListComponent {
        ...
        this.trainingService.getAllStates().subscribe({
            next: (response) => {
                if (response.trainmentObjectives) {
                    this.states.set(response.trainmentObjectives);
                }
    ...
    onEdit(id: number) {
        this.router.navigate(['/admin/catalog/states/form'], { queryParams: { id } });
    }

    onCreate() {
        this.router.navigate(['/admin/catalog/states/form']);
    }
}
